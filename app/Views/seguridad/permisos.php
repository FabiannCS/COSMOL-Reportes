...
<div class="container-fluid p-0">
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-2 mb-3 mb-md-4">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                ...
                <?php if ($esRolProtegido): ?>
                    <span class="badge bg-secondary-subtle text-secondary border border-secondary border-opacity-25 rounded-pill px-2">
        ...
        <div>
            <a href="/seguridad/roles" class="btn btn-outline-secondary btn-sm d-inline-flex align-items-center gap-1 shadow-sm w-100 w-sm-auto">
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-3">
            <div class="text-muted small fw-semibold text-uppercase mb-2" style="font-size: 0.8rem; letter-spacing: 0.5px;">Seleccionar Rol a Configurar:</div>
            <ul class="nav nav-pills gap-2 flex-wrap">
                <?php foreach ($roles as $r): ?>
                    ...
                    ?>
                    <li class="nav-item">
                        <a class="nav-link <?= $esActivo ? 'active shadow-sm' : 'bg-light text-dark border' ?> d-flex align-items-center gap-2 px-3 py-2" 
    ...
    <form action="/seguridad/roles/permisos" method="POST" id="formPermisos">
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4 border-start border-primary border-4">
            <div class="card-body p-3 p-md-4 d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                <div>
                    ...
                    <div>
                        <span class="badge bg-primary-subtle text-primary border border-primary border-opacity-25 rounded-pill px-3 py-1" id="contadorPermisosBadge">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2 w-100 w-md-auto justify-content-start justify-content-md-end">
                    <?php if (hasPermission('roles.permisos')): ?>
                        <button type="button" class="btn btn-sm btn-outline-secondary d-inline-flex align-items-center gap-1" id="btnMarcarTodosGlobal">
                            <i class="bi bi-check-all"></i>Marcar Todos
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-secondary d-inline-flex align-items-center gap-1" id="btnDesmarcarTodosGlobal">
                            <i class="bi bi-dash-square"></i>Desmarcar Todos
                        </button>
                        <button type="submit" class="btn btn-sm btn-primary d-inline-flex align-items-center gap-1 shadow-sm px-3">
                    ...
                    <?php else: ?>
                        ...
                        <button type="button" class="btn btn-sm btn-secondary disabled" disabled title="No posee permiso para modificar la matriz de permisos">
                            <i class="bi bi-save"></i>
                            <span>Guardar Cambios</span>
                        </button>
                    <?php endif; ?>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                    <div class="card-header bg-white border-bottom py-3 px-4 d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-2">
                        ...
                        <div class="d-flex align-items-center gap-2 small">
                            <button type="button" class="btn btn-sm btn-link text-decoration-none p-0 text-primary fw-semibold btn-marcar-modulo" data-target="<?= $moduloKey ?>">
                                Marcar módulo
                            </button>
                                    ...
                                    <div class="p-3 border rounded-3 bg-light bg-opacity-50 h-100">
                                        <div class="form-check form-switch d-flex align-items-start gap-2 ps-0 mb-0">
                                            ...
                                            <label class="form-check-label flex-grow-1" for="permiso_<?= $idPermiso ?>" style="cursor: pointer;">
                                                <div class="fw-semibold text-dark"><?= htmlspecialchars($nombre, ENT_QUOTES, 'UTF-8') ?></div>
                                                <code class="small text-primary bg-white px-1 py-0 rounded border border-primary border-opacity-25"><?= htmlspecialchars($clave, ENT_QUOTES, 'UTF-8') ?></code>
                                                <?php if (!empty($desc)): ?>
        ...
        <div class="card border-0 shadow-sm rounded-4 mb-4 bg-white">
            <div class="card-body p-3 d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3">
                ...
                <?php if (hasPermission('roles.permisos')): ?>
                    <button type="submit" class="btn btn-sm btn-primary px-4 d-inline-flex align-items-center gap-1 shadow-sm">
                        <i class="bi bi-save"></i>
